...
void bserver::communicate_with_client(char *password, int port, char *key_path, char *keypub_path) {
    ...
    read_key_from_file(key_path, keypub_path);
...
void bserver::read_key_from_file(char *path, char *path2) {
    std::cout << "Loading key from: " << path << std::endl;
    std::cout << "Loading key from: " << path2 << std::endl;
    std::string item_name;
    std::ifstream nameFileout;
    std::ifstream nameFileout2;
    nameFileout.open(path);
    nameFileout2.open(path2);
    std::string line;
...
char* bserver::sign_msg(BIGNUM *msg_to_sign) {
    ...
    BIGNUM *r_ex = BN_new();
    BN_mul(r_ex, r_e, msg_to_sign, ctx);

    BIGNUM *y = BN_new();
    BN_mod_exp(y, r_ex, d, N, ctx);

    BIGNUM *r_inv = BN_new();
    BN_mod_inverse(r_inv, r, N, ctx);

    BN_mod_mul(y, y, r_inv, N, ctx);
...
int main(int argc, char*argv[]) {
    ...
    if(strcmp(argv[1], "sign") == 0) {
        ...
        server->communicate_with_client(argv[2], atoi(argv[3]), argv[4], argv[5]);
    }
    else {
